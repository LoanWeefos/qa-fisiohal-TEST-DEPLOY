...
describe("TC99 — Validar campo Frecuencia semanal de la actividad", function () {
  this.timeout(0);
  ...
  before(async () => {
    if (!global.__runningAll) {
      ...
      logger.info("\nEjecutando TC99 — Validar campo Frecuencia semanal de la actividad | 99 de 105\n");
    }
  ...
  it("El campo solo debe aceptar texto, inidicar con un mensaje de advertencia", async () => {
    ...
    let detalleInput = await global.driver.findElements(
      By.xpath("//input[@name='Frecuencia_semanal_de_la_actividad']")
    );

    assert.strictEqual(
      detalleInput.length,
      0,
      "El campo Frecuencia semanal de la actividad NO debería mostrarse cuando se selecciona 'No'"
    );
    ...
    assert.ok(
      ...
      "El campo Frecuencia semanal de la actividad debería mostrarse al seleccionar 'Si'"
    );
    ...
    const textoValido = "Tres veces por semana";
