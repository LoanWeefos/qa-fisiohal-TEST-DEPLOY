...
describe("TC61 — Validar campo ¿Hay antecedentes de cáncer en tu familia?", function () {
  this.timeout(0);
  ...
  beforeEach(async () => {
    if (!global.__runningAll) {
      ...
      logger.info("\nEjecutando TC61 — Validar campo ¿Hay antecedentes de cáncer en tu familia? | 61 de 105\n");
    }
  ...
  it("Comprobar elementos en el campo y solo elegir 1 elemento", async () => {
    ...
    const optionEls = await cancerSelect.findElements(By.xpath(".//option"));
    const uiOptions = [];
    ...
    assert.deepStrictEqual(
      ...
      `Opciones incorrectas en ¿Hay antecedentes de cáncer en tu familia?\nUI: ${JSON.stringify(uiOptions)}`
    );

    await cancerSelect.sendKeys("Si");
    await global.driver.sleep(300);

    let selectedValue = await cancerSelect.getAttribute("value");
    ...
    const opcionNo = await cancerSelect.findElement(
    ...
    selectedValue = await cancerSelect.getAttribute("value");
