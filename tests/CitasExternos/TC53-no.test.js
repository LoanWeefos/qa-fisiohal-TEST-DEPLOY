...
describe("TC48 — Validar campo obligatorio Material", function () {
  this.timeout(0);
  ...
  it("Debe mostrar error y no permitir avanzar sin seleccionar Material", async () => {
    ...
    const errorMessage = await global.helper.findOrFail(
      "//*[contains(text(),'Please select') or contains(text(),'Material')]"
    );

    const errorText = await errorMessage.getText();

    assert.ok(errorText, "No se muestra el mensaje de error esperado");
  });
