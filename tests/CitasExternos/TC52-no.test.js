...
describe("TC47 — Validar formato válido Cantidad usada", function () {
  this.timeout(0);
  ...
  it("Debe mostrar error al capturar letra en Cantidad usada", async () => {
    ...
    await cantidadInput.sendKeys("e");
    await global.driver.sleep(500);
    ...
    const errorMessage = await global.helper.findOrFail(
      "//*[contains(text(),'Please enter some valid input')]"
    );

    const errorText = await errorMessage.getText();

    assert.ok(
      errorText.includes("Please enter some valid input"),
      "No se mostró el mensaje de error por formato inválido en Cantidad usada"
    );
  });
