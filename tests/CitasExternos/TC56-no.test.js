...
describe("TC53 — Validar registro de cita con recurrencia mensual", function () {
  this.timeout(0);
  ...
  it("Debe registrar una cita recurrente mensual y reflejarse en el calendario", async () => {
    ...
    await recurrenceSelect.sendKeys("Monthly");
    await global.driver.sleep(800);
    ...
    const nextCalendarBtn = await global.driver.findElement(
      By.xpath("//button[contains(@class,'fc-next-button')]")
    );

    let monthsWithEvents = 0;
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (events.length > 0) monthsWithEvents++;
    ...
    assert.ok(
      monthsWithEvents >= 2,
      "La cita recurrente mensual no se reflejó correctamente en el calendario"
    );
  });
